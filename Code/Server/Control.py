...
class Control:
    def __init__(self):
        ...
        self.relax_max_time_limit = (
            60  # Set the maximum amount of time the robot dog is forced to rest mode
        )
    ...
    def condition(self):
        while True:
            try:
                if (
                    time.time() - self.move_timeout > self.relax_max_time_limit
                    and self.move_timeout != 0
                    and self.relax_flag == True
                ):
                    self.move_count = 0
                    self.move_timeout = time.time()
                if self.move_count < self.run_time_limit:
                    if (
                        (time.time() - self.timeout) > self.relax_min_time_limit
                        and self.timeout != 0
                        and self.relax_flag == False
                        and self.order[0] == ""
                    ):
                    ...
                    if self.move_flag != 1:
                        self.move_flag = 1
                    if self.move_count > (
                        self.run_time_limit + self.relax_max_time_limit
                    ):
                        self.move_count = 0
                        self.move_flag = 0
                    self.order = ["", "", "", "", ""]
    ...
    def IMU6050(self):
        ...
        while True:
            ...
            if (
                ...
                or (self.balance_flag == True and self.order[0] != "")
                or (self.move_count > self.run_time_limit)
            ):
